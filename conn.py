...
#   ,30-04-2021,3115.25,   3099,3132.05,3020,3035.65,3063.19,3072305,9.41E+14, 1942473,           0.6323,          18
    try:
        ...
        cursor.execute("INSERT INTO wipro(Date, Prev_Close, Open, High, Low, Last, Close, VWAP, Volume, Turnover) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                       (date, prev_close, open_value, high, low, last, close, vwap, volume, turnover))
...
#   ,30-04-2021,3115.25,   3099,3132.05,3020,3035.65,3063.19,3072305,9.41E+14, 1942473,           0.6323,          18
    ...
    try:
        ...
        cursor.execute("UPDATE wipro SET Prev_Close=?, Open=?, High=?, Low=?, Close=?, VWAP=?, Volume=?, Turnover=? WHERE id=?", 
                       (new_prev_close, new_open, new_high, new_low,new_last, new_close, new_vwap, new_volume, new_turnover, record_id))
        conn.commit()
...
def delete_record():
    ...
    try:
        ...
        cursor.execute("DELETE FROM wipro WHERE id=?", (record_id,))
        conn.commit()
